page 50112 "PO History"
{
    Caption = 'Purchase Order History';
    PageType = Document;
    RefreshOnActivate = true;
    SourceTable = "Purchase Header";
    SourceTableView = WHERE("Document Type" = CONST(Order), status = filter(Released), "Show Vessel GRN" = filter(true));
    ModifyAllowed = false;
    InsertAllowed = false;
    DeleteAllowed = false;
    layout
        ...
        area(content)
        {
            group(General)
            {
                ...
                field("Buy-from Contact"; Rec."Buy-from Contact")
                {
                    ...
                    trigger OnLookup(var Text: Text): Boolean
                    begin
                        //Rec.LookupBuyFromContact();       //PCPL-25/280723
                        CurrPage.Update();
                    end;
                }
            ...
            part(PurchLines; "PO History Subform")
            {
                ApplicationArea = Suite;
                ...
                group(Control71)
                {
                    ...
                    group(Control95)
                    {
                        ...
                        field("Pay-to Address"; Rec."Pay-to Address")
                        {
                            ApplicationArea = Basic, Suite;
                            Caption = 'Address';
                            // Editable = (PayToOptions = PayToOptions::"Custom Address") OR (Rec."Buy-from Vendor No." <> "Pay-to Vendor No.");
                            // Enabled = (PayToOptions = PayToOptions::"Custom Address") OR (Rec."Buy-from Vendor No." <> "Pay-to Vendor No.");
                            Importance = Additional;
                            QuickEntry = false;
...
#pragma warning restore
                ...
                action("Send Intercompany Purchase Order")
                {
                    ...
                    begin
                        if ApprovalsMgmt.PrePostApprovalCheckPurch(Rec) then begin
                            ...
                            //ICFeedback.ShowIntercompanyMessage(Rec, Rec."IC Transaction Document Type"::Order);
                        end;
                    end;
                }
                ...
                /*  action("Purchase Order Report")
                 {
                     ...
                     //PromotedCategory = Category_Category10;
                     // Promoted = true;
                     Image = Print;
                     ApplicationArea = all;
                     trigger OnAction()
        ...
        area(Promoted)
        {
            ...
            group(Category_Category10)
            {
                ...
                actionref(PurchaseOrderReport_Promoted; PurchaseOrderReport)
                {
    ...
    begin
        if not OrderPurchaseHeader.Get(Rec."Document Type", Rec."No.") then begin
            ...
            if PurchInvHeader.FindFirst() then begin
                //ICFeedback.ShowIntercompanyMessage(Rec, "IC Transaction Document Type"::Order);       //PCPL-25/280723
